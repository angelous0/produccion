...
import {
  ...
  Search, ArrowDownUp, ChevronDown, ChevronRight, Timer,
} from 'lucide-react';
...
const NIVEL_CONFIG = {
  ...
  lento:     { label: 'Lento',     color: 'bg-amber-100 text-amber-700 border-amber-200', rowClass: 'bg-amber-50/30' },
  aceptable: { label: 'Aceptable', color: 'bg-transparent text-muted-foreground border-transparent', rowClass: '' },
...
export const ReporteTiemposMuertos = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const params = filtroTerminados === 'todos' ? '?incluir_resueltos=true' : '';
  ...
  useEffect(() => { fetchData(); }, [filtroTerminados]);

  const filtered = useMemo(() => {
    if (!data) return [];
    let items = data.brechas;
    if (busqueda.trim()) {
      const q = busqueda.toLowerCase();
      items = items.filter(b =>
        (b.n_corte || '').toLowerCase().includes(q) ||
        (b.modelo || '').toLowerCase().includes(q) ||
        (b.servicio_anterior || '').toLowerCase().includes(q) ||
        (b.servicio_siguiente || '').toLowerCase().includes(q) ||
        (b.persona_anterior || '').toLowerCase().includes(q) ||
        (b.persona_siguiente || '').toLowerCase().includes(q)
      );
    }
    items.sort((a, b) => sortDesc ? b.dias_brecha - a.dias_brecha : a.dias_brecha - b.dias_brecha);
    return items;
    ...
    <div className="space-y-4" data-testid="reporte-tiempos-muertos">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Brechas entre servicios — lotes parados sin avanzar</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center rounded-lg border text-sm overflow-hidden">
            <button type="button" onClick={() => setFiltroTerminados('en_curso')} className={`px-3 py-1.5 text-xs font-medium transition-colors ${filtroTerminados === 'en_curso' ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`}>En espera</button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiCard label="Brechas" value={resumen.total || 0} icon={Timer} />
        ...
        <KpiCard label="Días perdidos" value={resumen.dias_perdidos || 0} icon={Clock} danger />
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            {filtroTerminados === 'en_curso'
              ? 'Sin lotes en espera entre servicios'
              : 'No hay brechas registradas entre servicios'}
          </CardContent>
        </Card>
      ) : (
        <Card>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-muted/60 border-b">
                  ...
                  <th className="text-left p-2.5 font-medium text-muted-foreground">Serv. Anterior</th>
                  ...
                  <th className="text-left p-2.5 font-medium text-muted-foreground">Serv. Siguiente</th>
                  <th className="text-left p-2.5 font-medium text-muted-foreground">Persona</th>
                  <th className="text-center p-2.5 font-medium text-muted-foreground">Inició</th>
                  <th
              ...
              <tbody>
                {filtered.map((b, idx) => {
                  const cfg = NIVEL_CONFIG[b.nivel] || NIVEL_CONFIG.ok;
                  return (
                    <tr key={`${b.registro_id}-${idx}`} className={`border-t hover:bg-muted/30 transition-colors ${cfg.rowClass}`} data-testid={`brecha-${b.n_corte}-${idx}`}>
                      <td className="p-2.5 font-mono font-semibold whitespace-nowrap">
                        {b.n_corte}
                        {b.urgente && <span className="ml-1 text-[9px] text-red-600 font-bold">URG</span>}
                      </td>
                      <td className="p-2.5 whitespace-nowrap">{b.modelo || '-'}</td>
                      <td className="p-2.5 whitespace-nowrap font-medium">{b.servicio_anterior}</td>
                      <td className="p-2.5 whitespace-nowrap text-muted-foreground">{b.persona_anterior || '-'}</td>
                      <td className="p-2.5 text-center whitespace-nowrap">{fmtDate(b.fecha_fin_anterior)}</td>
                      <td className="p-2.5 whitespace-nowrap font-medium">{b.servicio_siguiente}</td>
                      <td className="p-2.5 whitespace-nowrap text-muted-foreground">{b.persona_siguiente || '-'}</td>
                      <td className="p-2.5 text-center whitespace-nowrap">
                        {b.en_espera ? <span className="text-amber-600 font-medium">Sin iniciar</span> : fmtDate(b.fecha_inicio_siguiente)}
                      </td>
                      <td className={`p-2.5 text-center font-mono font-bold whitespace-nowrap ${
                        b.dias_brecha >= 7 ? 'bg-red-100 text-red-700' :
                        b.dias_brecha >= 3 ? 'bg-amber-100 text-amber-700' : ''
                      }`}>
                        {b.dias_brecha}
                      </td>
                      <td className="p-2.5 text-center">
                        {b.nivel === 'ok' || b.nivel === 'aceptable' ? (
                      ...
                      <td className="p-2.5 text-center">
                        <Button
                          ...
                          onClick={() => navigate(`/registros/${b.registro_id}`)}
